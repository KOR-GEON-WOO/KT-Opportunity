    ...
    <aside className="lead-list panel">
      ...
      <div className="lead-list-scroll">
        {items.map((store) => {
          ...
          return (
            <button key={store.storeId} type="button" className={selected ? 'lead-row selected' : 'lead-row'} onClick={() => selectStore(store.storeId)}>
      ...
      {totalPages > 1 && <div className="pagination"><button disabled={page === 1} onClick={() => setPage((v) => v - 1)}>이전</button><span>{page} / {totalPages}</span><button disabled={page === totalPages} onClick={() => setPage((v) => v + 1)}>다음</button></div>}
    </aside>
  );
}
