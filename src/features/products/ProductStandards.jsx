...
export default function ProductStandards() {
  ...
  return (
    <div className="page-stack products-page">
      ...
      {state.status === 'loading' && <section className="panel skeleton-panel"><SkeletonBlock rows={4} label="상품 기준을 불러오는 중" /></section>}
      {state.status === 'error' && <div className="global-error error-with-action" role="alert"><span>{state.error}</span><button type="button" onClick={() => setReloadToken((value) => value + 1)}>다시 불러오기</button></div>}
