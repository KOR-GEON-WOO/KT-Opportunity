...
export default function History({ onlyFollowUp = false }) {
  ...
  useEffect(() => {
    ...
    return () => { active = false; };
  }, [onlyFollowUp, reloadToken]);

  if (state.status === 'loading') return <div className="page-stack"><section className="panel history-panel skeleton-panel"><div className="panel-heading"><div><span className="eyebrow">CONSULTATION HISTORY</span><h3>상담 이력</h3></div></div><SkeletonBlock rows={5} label="상담 이력을 불러오는 중" /></section></div>;
  if (state.status === 'error') return <div className="page-stack"><div className="global-error error-with-action" role="alert"><span>{state.error}</span><button type="button" onClick={() => setReloadToken((value) => value + 1)}>다시 불러오기</button></div></div>;
