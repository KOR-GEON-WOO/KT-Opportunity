...
import OperationalNotice from '../../components/ui/OperationalNotice.jsx';
...
export default function FollowUp({ onNavigate }) {
    ...
    return (
      <div className="page-stack followup-page">
        ...
        <section className="panel readonly-summary">
          ...
          <div className="action-row"><button type="button" className="button subtle" onClick={() => onNavigate('history')}>상담 이력 보기</button><button type="button" className="button primary" onClick={() => { startNewSearch(); onNavigate('discovery'); }}>새 음식점 찾기 <Icon name="arrow" size={18} /></button></div>
        </section>
      </div>
    );
  }
  ...
  return (
    <div className="page-stack followup-page">
      <OperationalNotice message={error} context="save" onNavigate={onNavigate} />
      ...
      <section className="followup-grid">
        <article className="panel followup-form-panel">
          ...
          <div className="save-gate-status" role="status" aria-live="polite">
            <span className={form.saveApproved ? 'ready' : ''} aria-hidden="true">{form.saveApproved ? '✓' : '!'}</span>
            <p><strong>{form.saveApproved ? '저장 준비 완료' : '최종 확인이 필요합니다'}</strong><small>{form.saveApproved ? '승인 및 저장을 선택하면 상담 이력에 새 기록으로 누적됩니다.' : '위 내용을 확인한 뒤 최종 확인 체크박스를 선택해야 저장할 수 있습니다.'}</small></p>
          </div>
          <div className="action-row"><button type="button" className="button subtle" onClick={() => onNavigate(proposal ? 'proposal' : 'discovery')}>이전 화면</button><button type="button" className="button primary" disabled={!form.saveApproved || saving} onClick={save}>{saving ? '저장 중...' : '승인 및 저장'} {!saving && <Icon name="arrow" size={18} />}</button></div>
        </article>
